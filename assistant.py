...
def get_audio():
    ...
    try:
        ...
        assistant_speaks("Could not get you , Please speak again!")
        #return 0
        return get_audio()
...
def open_application(input): 
	...
	if "chrome" in input: 
		...
		os.startfile('C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Microsoft Office 2013\\Word 2007.lnk') 
		return

	elif "excel" in input: 
		assistant_speaks("Opening Microsoft Excel") 
		os.startfile('C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Microsoft Office 2013\\Excel 2007.lnk') 
		return
...
def process_text(input):
    try:
        if "who are you" in input or "define yourself" in input:
            ...
            speak = "I have been created by Mimansha Malviya."
            assistant_speaks(speak)
            return
            ...
            speak = """Well, there are 2 mental asylums in India. wanna go?"""
            assistant_speaks(speak)
            return
        ...
        ans = get_audio()
        if 'yes' in str(ans) or 'yeah' or'sure' or 'ok' in str(ans):
            search_web(input)
